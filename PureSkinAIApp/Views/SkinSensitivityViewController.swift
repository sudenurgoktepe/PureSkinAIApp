...
class SkinSensitivityViewController: UIViewController {
    ...
    private func setupSensitivityOptions() {
        let sensitivities: [(title: String, icon: String, description: String, type: SkinSensitivity)] = [
            ("Hiç hassas değil", "face.smiling.fill", "Cildiniz güçlü ve dayanıklı.", .notSensitive),
            ("Biraz hassas", "face.neutral.fill", "Cildiniz zaman zaman hassasiyet gösterebilir.", .slightlySensitive),
            ("Çok hassas", "face.sad.fill", "Cildiniz sık sık hassasiyet gösterir.", .verySensitive)
        ]
    ...
    @objc private func sensitivityButtonTapped(_ sender: UIButton) {
        ...
        sensitivityStackView.arrangedSubviews.forEach { view in
            if let button = view as? UIButton {
                ...
                if let contentStack = button.subviews.first as? UIStackView {
                    // Metin stack view'ı bul
                    if let textStack = contentStack.arrangedSubviews.last as? UIStackView {
                        // Başlık rengini sıfırla
                        if let titleLabel = textStack.arrangedSubviews.first as? UILabel {
                            titleLabel.textColor = .black
                        }
                        // Açıklama rengini sıfırla
                        if let descriptionLabel = textStack.arrangedSubviews.last as? UILabel {
                            descriptionLabel.textColor = UIColor(red: 128/255, green: 128/255, blue: 128/255, alpha: 1.0)
                        }
                    }
                }
        ...
        if let contentStack = sender.subviews.first as? UIStackView {
            ...
            if let textStack = contentStack.arrangedSubviews.last as? UIStackView {
                ...
                // Açıklama rengini değiştir
                if let descriptionLabel = textStack.arrangedSubviews.last as? UILabel {
                    descriptionLabel.textColor = UIColor(red: 76/255, green: 175/255, blue: 80/255, alpha: 0.7)
                }
            }
        }
